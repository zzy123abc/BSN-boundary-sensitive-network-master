...
def readData(video_name):
    dir = "/gruntdata/disk1/data/audiofeatures/"
    video_name = "v_" + video_name
    if os.path.exists(dir + video_name + ".mp4.pkl.pkl"):
        path = dir + video_name + ".mp4.pkl.pkl"
        video_name = video_name + ".mp4.pkl"
    elif os.path.exists(dir + video_name + ".mkv.pkl.pkl"):
        path = dir + video_name + ".mkv.pkl.pkl"
        video_name = video_name + ".mkv.pkl"
    else:
        path = dir + video_name + ".webm.pkl.pkl"
        video_name = video_name + ".webm.pkl"
    new_data = cp.load(open(path))
    if new_data.keys() == []:
        new_data = np.zeros((1,128))
    else:
        new_data = new_data[video_name][0]
    return new_data
...
for videoName in videoNameList:
    ...
    outDf.to_csv("./audio_csv_mean_100/"+"v_"+videoName+".csv",index=False)
