...
def BSN_Train_PEM(opt):
    ...
    train_loader = torch.utils.data.DataLoader(ProposalDataSet(opt,subset="train"),
                                                batch_size=model.module.batch_size, shuffle=True,
                                                num_workers=32, pin_memory=True,drop_last=True,collate_fn=collate_fn)
...
def Fusion_TEM_results(opt):
    dir1 = 'c3d_output/TEM_results/'
    dir2 = 'resnet_output/TEM_results/'
    files = os.listdir(dir1)
    for file in files:
        df1 = pd.read_csv(dir1 + file)
        df2 = pd.read_csv(dir2 + file)
        res = (df1 + df2) / 2
        res.to_csv("./output/TEM_results/" + file, index=False)
...
def main(opt):
    ...
    if opt["module"] == "Fusion":
        if not os.path.exists("output/TEM_results"):
            os.makedirs("output/TEM_results")
        print "Fusion: start fusion TEM_results"
        Fusion_TEM_results(opt)
        print "Fusion: finish fusion TEM_results"
    elif opt["module"] == "PGM":
