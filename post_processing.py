...
def getDatasetDict(opt):
    ...
    for i in range(len(database.keys())):
        ...
        if video_info['subset'] == 'testing' and opt["pem_inference_subset"] == 'testing':
            video_dict[video_name]=video_new_info
        if "v_" + video_name in validation_anno.keys() and opt["pem_inference_subset"] == 'validation':
            video_dict[video_name]=video_new_info
    print "%s subset video numbers: %d" %(opt["pem_inference_subset"],len(video_dict))
    return video_dict
