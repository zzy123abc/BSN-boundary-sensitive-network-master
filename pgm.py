...
def generateProposals(opt,video_list,video_dict):
    training_anno = load_json('/gruntdata/disk1/data/captions/train.json')
    validation_anno = load_json('/gruntdata/disk1/data/captions/val_1.json')
    ...
    for video_name in video_list:
        ...
        min_start = min(start_scores)
        min_end = min(end_scores)
        gap_start = max_start - min_start
        gap_end = max_end - min_end
        ...
        for idx in range(1,tscale-1):
            ...
            elif start_scores[idx] > (peak_thres * gap_start) + min_start:
                start_bins[idx]=1
        ...
        for idx in range(1,tscale-1):
            ...
            elif end_scores[idx] > (peak_thres * gap_end) + min_end:
                end_bins[idx]=1
        ...
        if "v_" + video_name in training_anno.keys():
            video_labels = training_anno["v_"+video_name]['timestamps']
        if "v_" + video_name in validation_anno.keys():
            video_labels = validation_anno["v_"+video_name]['timestamps']
        ...
        try:
            ...
            for j in range(len(video_labels)):
                ...
                gt_xmins.append(tmp_info[0]/corrected_second)
                gt_xmaxs.append(tmp_info[1]/corrected_second)
...
def getDatasetDict(opt):
    ...
    for i in range(len(database.keys())):
        ...
        video_new_info = {}
        video_new_info['subset'] = database[video_name]['subset']
...
def PGM_proposal_generation(opt):
    video_dict = load_json('Evaluation/data/activity_net.v1-3.min.json')
    video_dict = video_dict['database']
